...
const TopMain1: NextPage<TopMain1Type> = ({
  ...
  const searchBarContainerStyle: CSSProperties = useMemo(() => {
    return {
      overflow: searchLabelOverflow,
    };
  }, [searchLabelOverflow]);
  ...
  return (
    <header
      className={`self-stretch flex flex-row items-start justify-start max-w-full z-[2] text-left text-xl text-dimgray-600 font-inter ${className}`}
      style={topMainStyle}
    >
      <div
        className="flex-1 rounded-tl-none rounded-tr-3xs rounded-b-none [background:linear-gradient(180deg,_#fff,_#f3f3f7_66.67%,_rgba(243,_243,_247,_0.88))] overflow-hidden flex flex-row items-start justify-start pt-[38px] pb-6 pl-[46px] pr-[45px] box-border gap-7 max-w-full"
